...
const urgencyClass = computed(() => {
  if (daysUntil.value < 0) return "text-n-ruby-11 font-semibold";
  if (daysUntil.value <= 1) return "text-n-orange-11 font-semibold";
...
<template>
  <div
    class="bg-n-solid-1 border border-n-amber-6 rounded-xl p-4"
  >
    <div class="flex items-start gap-3">
      <div
        class="w-10 h-10 bg-n-amber-3 rounded-xl flex items-center justify-center text-xl shrink-0"
      >
        📅
      </div>
      <div class="flex-1 min-w-0">
        ...
        <p class="text-sm text-n-slate-11 mt-1 line-clamp-3">
          {{ schedule.description }}
        </p>
        <div class="flex items-center gap-3 mt-2 text-xs text-n-slate-10">
